...
class GeneratorTransitionState
    : public CloneableTransitionState<GeneratorTransitionState> {
 public:
  // Create a SyntaxNetTransitionState to wrap this nlp_saft::ParserState.
  GeneratorTransitionState(std::unique_ptr<GeneratorState> generator_state,
  ...
  std::unique_ptr<GeneratorTransitionState> Clone() const override;
  ...
  // Accessor for the underlying nlp_saft::GeneratorState.
  GeneratorState *generator_state() { return generator_state_.get(); }
 ...
 private:
  // Underlying GeneratorState object that is being wrapped.
  std::unique_ptr<GeneratorState> generator_state_;
