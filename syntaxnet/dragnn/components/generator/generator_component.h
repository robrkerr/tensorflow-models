...
#include "dragnn/components/generator/generator_link_feature_extractor.h"
...
class GeneratorComponent : public Component {
 ...
 private:
  ...
  bool IsAllowed(GeneratorTransitionState *state, int action) const;

  // Returns true if this state is final
  bool IsFinal(GeneratorTransitionState *state) const;

  // State advance function for this component.
  void Advance(GeneratorTransitionState *state, int action,
               Beam<GeneratorTransitionState> *beam);

  // Creates a new state for the given nlp_saft::SentenceExample.
  std::unique_ptr<GeneratorTransitionState> CreateState(
      SyntaxNetSentence *example);

  // Creates a newly initialized Beam.
  std::unique_ptr<Beam<GeneratorTransitionState>> CreateBeam(int max_size);

  // Transition system.
  std::unique_ptr<GeneratorTransitionSystem> transition_system_;
  ...
  // State search beams
  std::vector<std::unique_ptr<Beam<GeneratorTransitionState>>> batch_;
