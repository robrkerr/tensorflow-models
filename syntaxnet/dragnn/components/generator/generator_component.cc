#include "dragnn/components/generator/generator_component.h"
...
void GeneratorComponent::InitializeComponent(const ComponentSpec &spec) {
...
std::unique_ptr<Beam<GeneratorTransitionState>> GeneratorComponent::CreateBeam(
    int max_size) {
...
void GeneratorComponent::InitializeData(
  ...
  for (int batch_index = 0; batch_index < sentences->data()->size();
    ...
    std::vector<std::unique_ptr<GeneratorTransitionState>> initial_states;
    if (parent_states.empty()) {
      ...
      for (int i = 0; i < num_states; ++i) {
        std::unique_ptr<GeneratorTransitionState> state(
            CreateState(&(sentences->data()->at(batch_index))));
...
bool GeneratorComponent::IsReady() const { return input_data_ != nullptr; }

string GeneratorComponent::Name() const {
  return "SyntaxNet-backed beam parser";
}

int GeneratorComponent::BatchSize() const { return batch_.size(); }

int GeneratorComponent::BeamSize() const { return max_beam_size_; }

int GeneratorComponent::StepsTaken(int batch_index) const {
  return batch_.at(batch_index)->num_steps();
}

int GeneratorComponent::GetBeamIndexAtStep(int step, int current_index,
                                           int batch) const {
  return batch_.at(batch)->FindPreviousIndex(current_index, step);
}

int GeneratorComponent::GetSourceBeamIndex(int current_index, int batch) const {
  return batch_.at(batch)->FindPreviousIndex(current_index, 0);
}

std::function<int(int, int, int)> GeneratorComponent::GetStepLookupFunction(
    const string &method) {
  if (method == "shift-reduce-step") {
    // TODO(googleuser): Describe this function.
    return [this](int batch_index, int beam_index, int value) {
      GeneratorTransitionState *state =
...
void GeneratorComponent::AdvanceFromPrediction(const float transition_matrix[],
...
int GeneratorComponent::GetFixedFeatures(
...
void GeneratorComponent::FinalizeData() {
...
void GeneratorComponent::ResetComponent() {
  for (auto &beam : batch_) {
    beam->Reset();
  }
  input_data_ = nullptr;
...
std::unique_ptr<GeneratorTransitionState> GeneratorComponent::CreateState(
...
void GeneratorComponent::InitializeTracing() {
  ...
  for (auto &beam : batch_) {
    for (int beam_idx = 0; beam_idx < beam->size(); ++beam_idx) {
      GeneratorTransitionState *state = beam->beam_state(beam_idx);
...
void GeneratorComponent::DisableTracing() {
  do_tracing_ = false;
  feature_extractor_.set_add_strings(false);
}

void GeneratorComponent::AddTranslatedLinkFeaturesToTrace(
...
std::vector<std::vector<ComponentTrace>> GeneratorComponent::GetTraceProtos()
    const {
...
REGISTER_DRAGNN_COMPONENT(GeneratorComponent);
